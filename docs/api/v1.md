...
**Note:** Capabilities come from the device's associated driver. `supports_bypass` is true only for agent-controlled devices (passive driver): bypass is read by agents only and has no effect on push-controlled devices (Aqara, Kidslox, Notify). The `emoji` field is optional and only returned when a custom emoji override is configured. When absent, clients should derive the emoji from the device `type`.
...
- `401` - Unauthorized
- `409` - Lockdown is active (`LOCKDOWN_ACTIVE`)
...
- `409` - Lockdown is active (`LOCKDOWN_ACTIVE`, extend only)
...
Global lockdown blocks creation and extension of ALL sessions (child UI, bot, API) until manually disabled. Daily time quotas are not reset - remaining minutes stay available after unlock. See `docs/features/lockdown.md` for details.
...
{
  "enabled": true,
  "enabled_at": "2026-09-06T18:30:00+03:00",
  "enabled_by": "api"
}
```

When disabled, `enabled_at` and `enabled_by` are `null`.

#### POST /v1/lockdown/enable

Enable lockdown. Stops all active sessions and blocks new ones until disabled. Idempotent.
...
```
`enabled_by` is a free-form audit label (defaults to `"api"`).
...
`failed_sessions` counts active sessions that could not be stopped (e.g. a device driver error); the scheduler ends them within a minute.

**Error Responses:**
- `401` - Unauthorized
- `500` - Failed to enable lockdown

#### POST /v1/lockdown/disable

Disable lockdown. Sessions can be started again using remaining daily time.
...
**Error Responses:**
- `401` - Unauthorized
- `500` - Failed to disable lockdown
...
Returns the Windows agent build published by CI. Used by the agent updater
(`metron-win-agent.exe -mode updater`, SYSTEM task every 5 minutes). Same Bearer token auth as
`/v1/agent/session`. See [Windows Agent Auto-Update](../drivers/windows-agent-autoupdate.md).

**Query Parameters:**
- `version` (optional): Version of the requesting agent (logged only)

**Response:** (200 OK)
```json
{
  "version": "20260923.1939-0176dec",
  "sha256": "311a5b408f1af95e3b350571f938b655576f06487d8ee91a825e0b16e0eb4d5a",
  "size": 9290752,
  "signature": "xsraiRTWmPXG...",
  "built_at": "2026-09-23T19:40:00Z"
}
```

The updater installs the build when `sha256` differs from the installed binary. `signature` is an
ed25519 signature of `metron-win-agent:<version>:<sha256>:<size>`, required when the agent was built
with a public key.

**Error Responses:**
- `401` - Missing or invalid authorization token
- `404` - No update published (`UPDATE_NOT_AVAILABLE`)

#### GET /v1/agent/update/download

Streams the published agent binary (`application/octet-stream`). Returns `404` when nothing is published.
...
Get the active bypass for a device. Used by the Telegram bot to show bypass status.

**Response (bypass active):** (200 OK)
```json
{
  "device_id": "win-pc1",
  "enabled": true,
...
`expires_at` is omitted for an indefinite bypass.

**Response (no active bypass):** (404 Not Found)
```json
{
...
An expired bypass is cleared on read and also returns 404.

**Example:**
```bash
curl http://localhost:8080/v1/devices/win-pc1/bypass \
...
- `expires_in_minutes` (optional): When bypass should expire. If omitted, bypass is indefinite (the Telegram bot never sends an indefinite bypass; its longest option is "until end of day").
...
Telegram buttons:
- Step 1: 👧 Alice | 👦 Bob | 👨‍👩‍👧 Shared (All)
- Step 2: 📺 Living Room TV | 📺 Bedroom TV
- Step 3: ⏱ +5 | +15 | +30 | +60 | +120 mins
